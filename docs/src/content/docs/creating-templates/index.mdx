...
The template file — `template/hello.txt.die`:
...
The `template/` directory contains your project files. Files ending in `.die` are rendered through the [Tera template engine](https://keats.github.io/tera/), then have the `.die` suffix stripped. Everything else is copied as-is.
...
`ci_provider` is only prompted when `use_ci` is true. The `when` field takes a bare Tera boolean expression — the same syntax used in `{% if ... %}` blocks.
...
This excludes `src/cli.py.die` when the user says no to the CLI entry point. The glob matches both the `.die` source and any related files.
...
A `Cargo.toml.die` from a Rust CLI template:
